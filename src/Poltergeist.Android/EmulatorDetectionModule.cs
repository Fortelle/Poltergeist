...
public class EmulatorDetectionModule : MacroModule
{
    ...
    private static bool DetectMumu12(IPreparableProcessor processor)
    {
        var subkey = RegistryUtil.FindInstalledApp("MuMuPlayer-12.0");
        if (subkey is null)
        {
            return false;
        }
        ...
        var emulatorpath = Path.Combine(installPath, @"shell\MuMuPlayer.exe");
        if (!File.Exists(emulatorpath))
        {
            return false;
        }
